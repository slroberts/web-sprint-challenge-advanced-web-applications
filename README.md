...
A token is used to readily tell the client that it issued the token. It can also read the token and make decisions for data transfer based on the client’s permission.

2. What steps can you take in your web apps to keep your data secure?

Use JSON Web Tokens as the authentication token. These tokens are cryptic test, which can be stored on the client-side using local storage or session storage.

3. Describe how web servers work.

They process incoming network request via HTTP protocols. It primary function is to store and deliver web pages to clients.
